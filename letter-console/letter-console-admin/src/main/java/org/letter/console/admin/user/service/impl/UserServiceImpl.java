...
public class UserServiceImpl implements UserService {
	...
	private final UserOnlineService onlineUserService;
...
//			cacheProxy.del(CacheKey.DATA_USER + resources.getId());
//			cacheProxy.del(CacheKey.MENU_USER + resources.getId());
//			cacheProxy.del(CacheKey.ROLE_AUTH + resources.getId());
//        }
//        // 修改部门会影响 数据权限
...
//            map.put("岗位", userDTO.getJobs().stream().map(JobSmallDto::getName).collect(Collectors.toList()));
	...
	public void delCaches(Long id, String username) {
		cacheProxy.del(CacheKey.USER_ID + id);
		flushCache(username);
	}
